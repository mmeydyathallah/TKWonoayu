...
const char* API_TOKEN = "ISI_TOKEN_RFID_DARI_ENV";

// ================== NANO ESP32-C6 PIN ==================
// Nano-style board: gunakan pin label Arduino agar lebih mudah dipasang.
// I2C: PN532 dan LCD 16x2 I2C dipasang paralel pada SDA/SCL.
const int I2C_SDA_PIN = A4;
const int I2C_SCL_PIN = A5;
const int BUZZER_PIN = D7; // active buzzer: + ke D7, - ke GND

// ================== LCD ==================
LiquidCrystal_I2C lcd(0x27, 16, 2); // ganti ke 0x3F jika alamat LCD berbeda
...
void loop() {
  ...
  if (uidHex == lastUid && (nowMs - lastTapMs) < TAP_COOLDOWN_MS) return;

  lastUid = uidHex;
  lastTapMs = nowMs;

  showLCD("Kartu: " + uidHex.substring(0, 8), "Kirim data...");
  beep(1, 60, 0);
  ...
  if (!sent) {
    ...
    if (eventType == "masuk") eventLabel = "MASUK";
    if (eventType == "pulang") eventLabel = "PULANG";
    if (eventType == "sudah_tercatat") eventLabel = "SUDAH ADA";
