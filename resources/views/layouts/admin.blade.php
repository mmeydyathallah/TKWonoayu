...
<head>
    ...
    <title>{{ $title ?? 'Admin Panel - TK Wonoayu' }}</title>
...
<body data-theme="tkwonoayu" class="bg-surface text-on-surface flex min-h-screen">
    ...
    <main class="relative z-10 flex-1 md:ml-64 p-4 md:p-8 overflow-y-auto">
        ...
        <header class="md:hidden flex justify-between items-center mb-8 bg-surface-container-lowest p-4 rounded-xl shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <h1 class="font-headline font-bold text-amber-600">Admin Panel</h1>
            </div>
        ...
        <aside id="mobileMenu" class="md:hidden fixed inset-y-0 left-0 z-50 w-72 max-w-[85%] bg-surface-container-lowest shadow-2xl transform -translate-x-full transition-transform duration-300 overflow-y-auto">
            <div class="flex items-center justify-between px-4 py-4 border-b border-slate-200">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="font-headline text-base font-bold text-slate-900">Admin Panel</p>
            ...
            <div class="flex flex-col gap-2 p-4">
                <a class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.dashboard') ? 'bg-amber-500 text-white rounded-2xl shadow-lg shadow-amber-500/30 scale-[1.02]' : 'text-slate-600 hover:bg-slate-100 rounded-2xl' }} transition-all duration-300" href="{{ route('admin.dashboard') }}">
                ...
                <a class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.pengguna.*') ? 'bg-amber-500 text-white rounded-2xl shadow-lg shadow-amber-500/30 scale-[1.02]' : 'text-slate-600 hover:bg-slate-100 rounded-2xl' }} transition-all duration-300" href="{{ route('admin.pengguna.index') }}">
                    <span class="material-symbols-outlined text-[20px]" style="{{ request()->routeIs('admin.pengguna.*') ? 'font-variation-settings: \'FILL\' 1;' : '' }}">manage_accounts</span>
                    <span class="font-bold text-xs">Manajemen Pengguna</span>
                </a>
                <div class="my-4 border-t border-slate-200"></div>
                <p class="px-4 text-[10px] font-black text-amber-600 uppercase tracking-[0.2em] mb-1">Portal Guru</p>
                <a class="flex items-center gap-3 px-4 py-3 text-slate-600 hover:bg-slate-100 rounded-2xl transition-all duration-300" href="{{ route('guru.dashboard') }}">
    ...
    <script>
        function toggleMobileAdminMenu(open) {
